 ...
 * @version 1.0.0
 * 
 * A simple script to test the proxy wrapper directly with TypeScript.
 */
 ...
 */
function findToolHandler(server: any, toolName: string): Function | null {
  // Try different ways to access the tools
  if (server._tools && server._tools.get) {
    return server._tools.get(toolName).handler;
  }
  
  if (server.tools && server.tools.get) {
    return server.tools.get(toolName).handler;
  }
